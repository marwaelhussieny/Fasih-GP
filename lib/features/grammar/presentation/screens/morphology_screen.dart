// lib/features/grammar/presentation/screens/grammar_morphology_screen.dart - UPDATED WITH BACKEND INTEGRATION
...
class GrammarMorphologyScreen extends StatefulWidget {
  const GrammarMorphologyScreen({Key? key}) : super(key: key);

  @override
  State<GrammarMorphologyScreen> createState() => _GrammarMorphologyScreenState();
}

class _GrammarMorphologyScreenState extends State<GrammarMorphologyScreen>
  ...
  Future<void> _performMorphology() async {
    if (_textController.text.trim().isEmpty) {
      _showSnackBar('الرجاء إدخال نص للتحليل الصرفي.', Colors.orange);
      return;
    }
    ...
    // Perform morphology analysis using the backend
    await provider.performMorphology(_textController.text.trim(), 'detailed');

    if (provider.error == null) {
      _resultAnimationController.forward(from: 0.0);
      _showSnackBar(provider.getSuccessMessage('morphology'), Colors.green);
    } else {
      _showSnackBar(provider.getErrorMessage('morphology', provider.error!), Colors.red);
    }
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 4.h),
                Text(
                  'أدخل نص لتحليل بنية الكلمات وتصريفها',
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'مثال: الطالب المجتهد يذهب إلى المدرسة',
  ...
  Widget _buildActionButton(GrammarMorphologyProvider provider, ColorScheme colorScheme, TextTheme textTheme) {
    return Container(
      ...
      child: ElevatedButton(
        onPressed: provider.isLoading ? null : _performMorphology,
  ...
  Widget _buildResultsSection(
    ...
    if (provider.morphologyResults.isEmpty && !provider.isLoading) {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 15.h),
              ...provider.morphologyResults.asMap().entries.map((entry) {
                ...
                return TweenAnimationBuilder<double>(
                  ...
                  builder: (context, animation, child) {
                    return Transform.translate(
                      ...
                      child: Opacity(
                        opacity: animation,
                        child: _buildMorphologyResultCard(result, colorScheme, textTheme, isDarkMode),
                      ),
                    );
                  },
                );
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                _buildDetailRow('النوع:', result.type, textTheme, isDarkMode, colorScheme),
                if (result.state.isNotEmpty) ...[
                  SizedBox(height: 6.h),
                  _buildDetailRow('الحالة:', result.state, textTheme, isDarkMode, colorScheme),
                ],
                if (result.root.isNotEmpty && result.root != 'N/A') ...[
                  SizedBox(height: 6.h),
                  _buildDetailRow('الجذر:', result.root, textTheme, isDarkMode, colorScheme),
                ],
              ],
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          SizedBox(height: 20.h),
          Text(
            'نتائج التحليل الصرفي ستظهر هنا',
          ...
          SizedBox(height: 8.h),
          Text(
            'أدخل نص واضغط على "التحليل الصرفي" للبدء',
            style: textTheme.bodySmall?.copyWith(
